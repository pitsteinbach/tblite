...
!> The #tblite_sqmbox_calculator::sqmbox_calculator collects the basic interactions
!> required to perform a tight-binding calculation.
module tblite_sqmbox_calculator
   ...
   use tblite_xtb_calculator, only : param_h0spec

   implicit none
   private

   public :: new_sqmbox_calculator
   ...
   type, public :: sqmbox_calculator
      !> Basis set definition
      type(basis_type) :: bas
      !> kinetic energy
      type(tb_hamiltonian) :: T(:)
      !> Coordination number for modifying the self-energies
      class(ncoord_type), allocatable :: ncoord(:)
      !> Repulsion energy interactions
      type(sqmbox_repulsion), allocatable :: nn_repulsion(:)
      !> Collection of all Coulombic interactions
      type(sqmbox_coulomb), allocatable :: coulomb(:)
      !> Halogen bonding correction TODO
   ...
   contains
      ...
      procedure :: pop
   end type sqmbox_calculator

contains


!> Create new SQMBox Hamiltonian calculator from parametrization data
subroutine new_sqmbox_calculator(calc, mol, param, error)
   !> Instance of the xTB calculator
   type(sqmbox_calculator), intent(out) :: calc
   !> Molecular structure data
   type(structure_type), intent(in) :: mol(:)
   !> Parametrization records, only on instance needed since mol objects must hove same elements
   ...
   !only one basis object needed per since the entries are independant of the coordinates
   call add_basis(calc, mol(1), param, irc) !done
   ! add Vnn which contains two 
   call add_ncoord(calc, mol, param) !creates the array of ncoord objects
  
   call add_kinetic(calc, mol, param, irc) !EHT
   call exit(1)
   !we need gamma from here on
   call add_nuclear_nuclear_repulsion(calc, mol, param, irc) 
   ! classical nuclear repulsion term, repulsion from reference occupation
...
end subroutine new_sqmbox_calculator
...
subroutine add_basis(calc, mol, param, irc)
   ...
   allocate(cgto(maxval(nsh_id), mol(1)%nid))
   do isp = 1, mol(1)%nid
      ang_idx = 0
      ortho = 0
      ir = irc(isp)
...
subroutine add_exchange(calc, mol, param, irc)
   ...
   if (allocated(param%exchange)) then 
      write(*,*) "Yes made it to the parameter file!!"
...
subroutine add_ncoord(calc, mol, param)
   ...
   if (allocated(param%hamiltonian%cn)) then
      do i = 1, size(mol)
         call new_ncoord(calc%ncoord(i), mol(i), cn_type=param%hamiltonian%cn)
      end do
   end if
end subroutine add_ncoord


subroutine add_kinetic(calc, mol, param, irc)
   !> Instance of the xTB evaluator
   type(sqmbox_calculator), intent(inout) :: calc
   !> Molecular structure data
   type(structure_type), intent(in) :: mol
   ...
   call new_hamiltonian(calc%kinetic, mol, calc%bas, new_param_h0spec(mol, param, irc))
end subroutine add_kinetic
...
subroutine update(self, mol)
   class(sqmbox_calculator), intent(inout) :: self
   type(structure_type), intent(in) :: mol
...
pure function variable_info(self) result(info)
   ...
   class(sqmbox_calculator), intent(in) :: self
...
end module tblite_sqmbox_calculator
